package com.mytechexp.binarysearch;

import java.util.ArrayList;
import java.util.Arrays;

/*
* Given a sorted array A of size N and a target value B, return the index (0-based indexing) if the target is found.
If not, return the index where it would be if it were inserted in order.
*
* */
public class PredictIndexOfTheElement {
    public int searchInsert(ArrayList<Integer> A, int B) {

        int start=0,end=A.size()-1,mid=0;


        while(start<=end)
        {
            mid=start+(end-start)/2;//avoid overflow safe condition

            if(A.get(mid)==B)
                return mid;

            if(B>A.get(mid) && B<=A.get(end))
                start=mid+1;
            else if(B>A.get(end))
                return end+1;
            else
                end=mid-1;
        }
        if(start>=end)
            return mid;
        return mid-1;
    }

    public static void main(String[] a)
    {
        PredictIndexOfTheElement predictIndexOfTheElement=new PredictIndexOfTheElement();
        Integer[] num=new Integer[]{3, 4, 18, 19, 20, 27, 28, 31, 36, 42, 44, 71, 72, 75, 82, 86, 88, 97, 100, 103, 105, 107, 110, 116, 118, 119, 121, 122, 140, 141, 142, 155, 157, 166, 176, 184, 190, 199, 201, 210, 212, 220, 225, 234, 235, 236, 238, 244, 259, 265, 266, 280, 283, 285, 293, 299, 309, 312, 317, 335, 341, 352, 354, 360, 365, 368, 370, 379, 386, 391, 400, 405, 410, 414, 416, 428, 433, 437, 438, 445, 453, 457, 458, 472, 476, 480, 485, 489, 491, 493, 501, 502, 505, 510, 511, 520, 526, 535, 557, 574, 593, 595, 604, 605, 612, 629, 632, 633, 634, 642, 647, 653, 654, 656, 658, 686, 689, 690, 691, 709, 716, 717, 737, 738, 746, 759, 765, 775, 778, 783, 786, 787, 791, 797, 801, 806, 815, 820, 822, 823, 832, 839, 841, 847, 859, 873, 877, 880, 886, 904, 909, 911, 917, 919, 937, 946, 948, 951, 961, 971, 979, 980, 986, 993};
        ArrayList<Integer> list=new ArrayList(Arrays.asList(num));
        System.out.println(predictIndexOfTheElement.searchInsert(list,902));
    }
}
